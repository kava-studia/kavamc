...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.LEADS_WEBHOOK_URL) {
      tasks.push(fetch(process.env.LEADS_WEBHOOK_URL, {
        method: "POST",
    ...
    if (tasks.length === 0) {
      return NextResponse.json({ error: "Форма ещё не подключена к Telegram. Сейчас открою прямой чат." }, { status: 503 });
    }

    const results = await Promise.allSettled(tasks);
    const delivered = results.some((result) => result.status === "fulfilled" && result.value.ok);
    if (!delivered) return NextResponse.json({ error: "Заявка не доставлена. Открываю Telegram." }, { status: 502 });
